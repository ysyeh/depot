...
Hanzi::utils - hanzi text processing utilities.
...
=head1 SYNOPSIS
	...
	#
	($whatis, @whatcouldbe) = Hanzi::utils::HzWhat($sometext);
	if ($whatis eq '?') {
	...
	#
	($whatis) = Hanzi::utils::HzWhat($sometext, 'gb', 'hz');
	if (!$whatis) {
	   print "It is neither gb nor hz.\n");
	} elsif ($whatis eq '?') {
	   # its neither 'gb' nor 'hz', could it be 'big5'?
	   ($isbig5) = Hanzi::utils::HzWhat($sometext, 'big5');
	   print "It's ", $isbig5 eq 'big5' ? "" : " not ", "big5.\n";
	} else {
	   print "it looks like $whatis.\n";
	   if ($whatis eq 'hz') {
	      ($isascii) = Hanzi::utils::HzWhat($sometext, 'ASCII');
	      if ($isascii eq 'ASCII') {
	   	 print "But of course, hz is 7bit code.\n";
	      } else {
		 # should never happen.
	   	 print "&*%@!! what a crap this Hanzi::utils::HzWhat() is.\n";
	      }
	   }
	}
...
C<Hanzi::utils::HzWhat> makes a semi-intelligent guess about what code set the 
given input text segment is. It returns an array of code sets with
descending likelihood, depends what code sets caller wishs C<Hanzi::utils::HzWhat> 
...
any target code set), the first element of the array which C<Hanzi::utils::HzWhat>
returns will be '?'. On the other hand, if C<Hanzi::utils::HzWhat> is certain
...
It makes no sense to apply C<Hanzi::utils::HzWhat> to heterogeneous text or 
binary data.

=head1 AUTHOR

Simon Yeh <syeh@csfcorp.com>, February, 1997.

=head1 VERSION

$Revision: '@(#)Hanzi:utils.pm 1.1 11 Apr 1997 15:10:27'
